...
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'products') THEN
    -- Create products table from scratch with all required columns
    CREATE TABLE products (
      ...
      media JSONB DEFAULT '{"images": [], "video": ""}'::JSONB,
      features TEXT,
      benefits TEXT,
      tagline TEXT,
    ...
    -- Create indexes for the new products table
    CREATE INDEX products_user_id_idx ON products(user_id);
    CREATE INDEX products_domain_id_idx ON products(domain_id);
    CREATE INDEX products_slug_idx ON products(slug);
  ELSE
    -- Products table exists, add columns if they don't exist
    BEGIN
      -- Add domain_id column if it doesn't exist
      IF NOT EXISTS (SELECT FROM information_schema.columns 
                     WHERE table_schema = 'public' AND table_name = 'products' 
                     AND column_name = 'domain_id') THEN
        ALTER TABLE products ADD COLUMN domain_id UUID REFERENCES domains(id) ON DELETE SET NULL;
      ...
      IF NOT EXISTS (SELECT FROM information_schema.columns 
                     WHERE table_schema = 'public' AND table_name = 'products' 
                     AND column_name = 'template_data') THEN
        ALTER TABLE products ADD COLUMN template_data JSONB DEFAULT '{}'::JSONB;
      END IF;
      
      IF NOT EXISTS (SELECT FROM information_schema.columns 
                     WHERE table_schema = 'public' AND table_name = 'products' 
                     AND column_name = 'seo') THEN
        ALTER TABLE products ADD COLUMN seo JSONB DEFAULT '{}'::JSONB;
      END IF;
      
      IF NOT EXISTS (SELECT FROM information_schema.columns 
                     WHERE table_schema = 'public' AND table_name = 'products' 
                     AND column_name = 'brand') THEN
        ALTER TABLE products ADD COLUMN brand JSONB DEFAULT '{}'::JSONB;
      END IF;
      
      IF NOT EXISTS (SELECT FROM information_schema.columns 
                     WHERE table_schema = 'public' AND table_name = 'products' 
                     AND column_name = 'testimonials') THEN
        ALTER TABLE products ADD COLUMN testimonials JSONB DEFAULT '[]'::JSONB;
      END IF;
      
      IF NOT EXISTS (SELECT FROM information_schema.columns 
                     WHERE table_schema = 'public' AND table_name = 'products' 
                     AND column_name = 'faq') THEN
      ...
      IF NOT EXISTS (SELECT FROM information_schema.columns 
        ...
        ALTER TABLE products ADD COLUMN media JSONB DEFAULT '{"images": [], "video": ""}'::JSONB;
      END IF;
      ...
      IF NOT EXISTS (SELECT FROM information_schema.columns 
                     WHERE table_schema = 'public' AND table_name = 'products' 
                     AND column_name = 'call_to_action') THEN
        ALTER TABLE products ADD COLUMN call_to_action JSONB DEFAULT '{}'::JSONB;
      END IF;
      
      IF NOT EXISTS (SELECT FROM information_schema.columns 
                     WHERE table_schema = 'public' AND table_name = 'products' 
                     AND column_name = 'colors') THEN
      ...
      IF NOT EXISTS (SELECT FROM information_schema.columns 
        ...
        ALTER TABLE products ADD COLUMN theme JSONB DEFAULT '{}'::JSONB;
      END IF;
    END;
  END IF;
END
...
-- Create multi-product stores table
...
-- Create store products junction table
...
-- Create index for faster store product lookups
CREATE INDEX IF NOT EXISTS store_products_store_id_idx ON store_products(store_id);
CREATE INDEX IF NOT EXISTS store_products_product_id_idx ON store_products(product_id);
...
BEGIN
  RETURN QUERY
  SELECT 
    p.id, p.user_id, p.name, p.description, p.price, p.template, p.slug, p.published, p.product_type,
...
BEGIN
  ...
  SELECT 
    ...
    s.contact_email, s.theme, s.template, d.subdomain,
    (
      SELECT jsonb_agg(
        jsonb_build_object(
          'id', p.id,
...
-- Create RLS policies for store_products table
ALTER TABLE store_products ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist to avoid conflicts
DROP POLICY IF EXISTS "Users can view their own store products" ON store_products;
DROP POLICY IF EXISTS "Users can insert their own store products" ON store_products;
DROP POLICY IF EXISTS "Users can update their own store products" ON store_products;
DROP POLICY IF EXISTS "Users can delete their own store products" ON store_products;
...
CREATE POLICY "Users can insert their own store products"
...
CREATE POLICY "Users can update their own store products"
...
CREATE POLICY "Users can delete their own store products"
  ...
  SELECT jsonb_build_object(
    ...
    'product', jsonb_build_object(
      ...
      'price', jsonb_build_object(
        'currency', COALESCE((p.template_data->>'currency'), 'USD'),
...
COMMENT ON TABLE stores IS 'Stores multi-product store information';
COMMENT ON TABLE store_products IS 'Junction table linking stores to their products';

...
export function PublishSuccess({ domain, productType }: PublishSuccessProps) {
  ...
  const handleGoToDashboard = () => {
    // Navigate to dashboard without sessionStorage cleanup
    ...
    >
      ...
      >
        <Button asChild className="bg-[#6F4E37] hover:bg-[#5D3D26] flex items-center gap-2">
          <Link href={`https://${domain}.productshowcase.com`} target="_blank">
        ...
        <Button 
          variant="outline" 
          className="border-[#ECB176] text-[#6F4E37] hover:bg-[#FED8B1]/20 flex items-center gap-2"
